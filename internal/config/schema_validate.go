...
func loadCompiledSchema() (*jsonschema.Schema, error) {
	schemaOnce.Do(func() {
		resource, err := jsonschema.UnmarshalJSON(strings.NewReader(configSchemaV1))
		if err != nil {
			schemaInitErr = fmt.Errorf("invalid embedded config schema: %w", err)
			return
		}
		compiler := jsonschema.NewCompiler()
		if err := compiler.AddResource("archguard.schema.json", resource); err != nil {
			schemaInitErr = fmt.Errorf("failed to add embedded config schema: %w", err)
			return
		}
		compiledSchema, schemaInitErr = compiler.Compile("archguard.schema.json")
		if schemaInitErr != nil {
			schemaInitErr = fmt.Errorf("failed to compile config schema: %w", schemaInitErr)
		}
	})
